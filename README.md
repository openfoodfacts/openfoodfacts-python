# Open Food Facts Python SDK

<div align="center">
  <img width="400" src="https://blog.openfoodfacts.org/wp-content/uploads/2022/05/EXE_LOGO_OFF_RVB_Plan-de-travail-1-copie-0-1-768x256.jpg" alt="Open Food Facts"/>
</div>

## Status

[![Project Status](https://opensource.box.com/badges/active.svg)](https://opensource.box.com/badges)
[![Build Status](https://travis-ci.org/openfoodfacts/openfoodfacts-python.svg?branch=master)](https://travis-ci.org/openfoodfacts/openfoodfacts-python)
[![codecov](https://codecov.io/gh/openfoodfacts/openfoodfacts-python/branch/master/graph/badge.svg)](https://codecov.io/gh/openfoodfacts/openfoodfacts-python)
[![Latest Version](https://img.shields.io/pypi/v/openfoodfacts.svg)](https://pypi.org/project/openfoodfacts)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://github.com/openfoodfacts/openfoodfacts-python/blob/master/LICENSE)

---

## Description

This is the official Python SDK for the
[Open Food Facts](https://world.openfoodfacts.org/) project.

The SDK provides a Python-friendly interface to the
[Open Food Facts API](https://openfoodfacts.github.io/openfoodfacts-server/api/),
allowing you to interact with Open Food Facts data without manually handling HTTP
requests or raw JSON responses.

It can be used in Python scripts, applications, or notebooks to explore, search,
and update food product data.

---

## Installation

Install the SDK using `pip` (recommended for most users):

```bash
pip install openfoodfacts
````

To install from source (useful if you want to contribute):

```bash
git clone https://github.com/openfoodfacts/openfoodfacts-python
cd openfoodfacts-python
pip install .  # Note the “.” at the end
```

---

## What you can do with this SDK

### 1. Get information about a product

Fetch product data using a barcode (for example, to retrieve the product name):

```python
import openfoodfacts

# User-Agent is mandatory
api = openfoodfacts.API(user_agent="MyAwesomeApp/1.0")

code = "3017620422003"
api.product.get(code, fields=["code", "product_name"])
# {'code': '3017620422003', 'product_name': 'Nutella'}
```

---

### 2. Perform text search on products

Search for products using keywords (for example, mineral water):

```python
api.product.text_search("mineral water")
# {"count": 3006628, "page": 1, "page_count": 20, "page_size": 20, "products": [{...}], "skip": 0}
```

The result includes pagination information and a list of matching products.

---

### 3. Create a new product or update an existing one

Create or update product data (authentication required):

```python
results = api.product.update({
    "code": CODE,
    "product_name_en": "blueberry jam",
    "ingredients_text_en": "blueberries, sugar, pectin, citric acid"
})
```

Here, `CODE` refers to the product barcode, and the remaining fields represent the
data to be created or updated.

---

## Helper utilities

In addition to direct API interactions, the SDK provides helper functions to make
working with Open Food Facts data easier, such as:

* Getting translations of taxonomized fields in a specific language
* Downloading and iterating over the Open Food Facts data dump
* Handling OCR results from Open Food Facts images generated by Google Cloud Vision

These utilities are especially useful when working with large datasets or
multilingual data.

---

## Further documentation

For advanced use cases and a complete overview of the SDK capabilities, refer to
the [usage guide](https://openfoodfacts.github.io/openfoodfacts-python/usage/).

---

## Third-party applications

If you use this SDK or plan to use it, make sure to read the
[REUSE](https://github.com/openfoodfacts/openfoodfacts-python/blob/develop/REUSE.md)
guidelines and ensure compliance with the OdBL licence, in addition to the MIT
licence of this package.

You are welcome to open a PR to add your application to the list.

---

## Contributing

Any help is welcome, as long as it does not break the continuous integration.

* Fork the repository
* Open a Pull Request directly on the `develop` branch
* A maintainer will review and integrate your changes

Maintainers:

* [Anubhav Bhargava](https://github.com/Anubhav-Bhargava)
* [Frank Rousseau](https://github.com/frankrousseau)
* [Pierre Slamich](https://github.com/teolemon)
* [Raphaël](https://github.com/raphael0202)

Contributors:

* Agamit Sudo
* [Daniel Stolpe](https://github.com/numberpi)
* [Enioluwa Segun](https://github.com/enioluwas)
* [Nicolas Leger](https://github.com/nicolasleger)
* [Pablo Hinojosa](https://github.com/Pablohn26)
* [Andrea Stagi](https://github.com/astagi)
* [Benoît Prieur](https://github.com/benprieur)
* [Aadarsh A](https://github.com/aadarsh-ram)

---

## Copyright and License

Copyright 2016–2024 Open Food Facts

The Open Food Facts Python SDK is licensed under the
[MIT License](https://github.com/openfoodfacts/openfoodfacts-python/blob/develop/LICENSE).

Other files that are not part of the SDK itself may be under a different license.
The project complies with the
[REUSE 3.3 specification](https://reuse.software/spec-3.3/).
